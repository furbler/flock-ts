...
(() => {
    ...
    //パラメータの初期化
    let param: Parameter = {
        cohesion_coef: 40, //群れの中心に向かう度合
        separation_coef: 40, //仲間を避ける度合
        alignment_coef: 10, //群れの平均速度に合わせる度合
        separation_thres: 40, //分離ルールの距離の閾値
        speed_limit: 3, //個体の制限速度
        sight_range: 100, //個体の視界距離
        cursor_radius: 50, //カーソルで動かす障害物の半径
        acceleration_limit: 0.1, //制限加速度(0は禁止)
    };
    ...
    function getValue(){
        ...
        let cursor: HTMLInputElement = <HTMLInputElement>document.getElementById("cursor_radius");
        //各個体の持つ情報を更新する
        boids.map((boid) => {
    ...
    function reset(){
        ...
        (<HTMLInputElement>document.getElementById("cursor_radius")).value = String(param.cursor_radius);
